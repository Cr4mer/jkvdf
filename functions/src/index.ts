...
async function isAdminSteamId(db: admin.firestore.Firestore, steamId: string): Promise<boolean> {
...
function isAllowedFaceitEndpoint(endpoint: string): boolean {
...
export const faceitProxy = functions.https.onCall(async (data, context) => {
  try {
 ...
 */
export const leetifyProxy = functions.https.onCall(async (data, context) => {
  try {
    ...
    const allowed =
      allowedPaths.some((p) => path === p || path.startsWith(p + '?') || path.startsWith(p + '/'));
    if (!allowed) {
      throw new functions.https.HttpsError('invalid-argument', 'Invalid path');
    }
    ...
    const tryPath = (param: string, value: string) => `${basePath}?${param}=${encodeURIComponent(value)}`;
    ...
    if (res.ok) {
      body = await res.json();
      const errMsg = body && typeof body === 'object' && 'error' in body && typeof (body as { error: unknown }).error === 'string' ? (body as { error: string }).error : '';
      if (errMsg && errMsg.toLowerCase().includes('identifier') && steamId64) {
        const toTry: { url: string }[] = [
          ...['steam64_id', 'steam_id_64', 'steamId64', 'steam_id', 'identifier'].map((param) => ({
        ...
        for (const { url: tryUrl } of toTry) {
          ...
          if (!res.ok) continue;
          body = await res.json();
          if (body && typeof body === 'object' && !('error' in body)) break;
        }
      }
    }
 ...
 * Wrapped in try/catch so all errors return HttpsError and get proper CORS headers.
 */
export const deleteTrainingSession = functions.https.onCall(async (data, context) => {
  try {
