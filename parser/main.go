...
func init() {
	// Allow envoy HTTP proxy injected at runtime.
	if proxy := os.Getenv("FACEIT_HTTP_PROXY"); proxy != "" {
...
var httpClient = &http.Client{
	...
	Transport: &http.Transport{
		...
		DialContext: func(ctx context.Context, network, addr string) (net.Conn, error) {
			...
			if lastErr == nil {
				lastErr = fmt.Errorf("no IP addresses resolved for %s", host)
			}
			return nil, lastErr
		},
	},
...
var fallbackResolvers = []string{"8.8.8.8:53", "1.1.1.1:53"}

var (
	manualDemoPath = flag.String("manual-demo", "", "path to a local demo file (.dem or .dem.zst)")
	manualMatchID  = flag.String("manual-match", "", "FACEIT match ID to process when using --manual-demo")
)
...
func resolveIP(ctx context.Context, host string) ([]net.IPAddr, error) {
	...
	var lastErr error
	for _, target := range fallbackResolvers {
...
func main() {
	...
	manualDemoAbs := ""
	if *manualDemoPath != "" {
		abs, err := filepath.Abs(*manualDemoPath)
		if err != nil {
			log.Fatalf("resolve manual demo path: %v", err)
		}
		if _, err := os.Stat(abs); err != nil {
			log.Fatalf("ensure manual demo accessible: %v", err)
		}
		if *manualMatchID == "" {
			log.Fatal("--manual-match is required when --manual-demo is provided")
		}
		manualMode = true
		manualDemoAbs = abs
	}
	...
	if manualMode {
		if _, ok := matchGroups[*manualMatchID]; !ok {
			log.Fatalf("manual match %s not found in data/matches.json; run ingest first", *manualMatchID)
		}
	}
	var newHighlights []highlight

	for matchID, group := range matchGroups {
		if manualMode && matchID != *manualMatchID {
			continue
		}
		if !manualMode {
		...
		var demoPath string
		if manualMode {
			demoURL = manualDemoAbs
			demoPath = manualDemoAbs
			log.Printf("  using local demo file %s", demoPath)
		} else {
			var err error
...
func getSignedDemoURL(rawURL string) (string, error) {
	...
	if resp.StatusCode == http.StatusNotFound {
		body, _ := io.ReadAll(resp.Body)
		log.Printf("  downloads API 404 for %s: %s", resource, strings.TrimSpace(string(body)))
...
func downloadDemo(matchID, rawURL string) (string, error) {
	...
	signedURL, err := getSignedDemoURL(rawURL)
	if err != nil {
		if errors.Is(err, ErrDemoNotReady) {
			log.Printf("  downloads API reports demo not ready for %s; will try raw URL", matchID)
		} else {
			log.Printf("  warning: could not get signed URL (%v); falling back to raw URL", err)
		}
	} else if signedURL != "" {
		candidates = append(candidates, candidate{url: signedURL, label: "signed"})
	}

	candidates = append(candidates, candidate{url: rawURL, label: "raw"})

	var lastErr error
	for _, cand := range candidates {
		path, err := downloadDemoFromURL(matchID, cand.url)
		if err == nil {
			if cand.label == "raw" && len(candidates) > 1 {
				log.Printf("  downloaded demo via raw URL fallback")
			}
			return path, nil
		}
		lastErr = err
		log.Printf("  %s download attempt failed: %v", cand.label, err)
	}

	if lastErr != nil {
		return "", fmt.Errorf("demo download failed for %s: %w", matchID, lastErr)
	}
	return "", fmt.Errorf("no demo URLs available for %s", matchID)
}

func downloadDemoFromURL(matchID, url string) (string, error) {
	var lastErr error
	...
	return "", fmt.Errorf("download failed after %d attempts", demoDownloadMaxAttempts)
}
